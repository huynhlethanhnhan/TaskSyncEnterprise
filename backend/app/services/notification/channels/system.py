...
class SystemChannel(BaseChannel):
    ...
    def send(self, db: Session, recipient_id: int, title: str, message: str, notification_id: int) -> bool:
        ...
        app_logger.info(f"[SYSTEM ALERT] To Employee ID {recipient_id}: Title: '{title}' | Message: '{message}'")
        ...
        app_logger.error(f"SystemChannel failed: Notification ID {notification_id} was not found in the database.")
        return False
