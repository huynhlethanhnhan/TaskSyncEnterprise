...
class TemplateContextBuilder:
    ...
    def build(extra_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        base = {
            ...
            "notification_time": datetime.now(timezone.utc).strftime(
                "%Y-%m-%d %H:%M:%S UTC"
            ),
            "brand": {
            ...
            "assets": {"logo_url": EmailAssetsManager.LOGO_URL},
        }
        if extra_context:
...
class EmailTemplateEngine:
    ...
    def _validate_template_name(self, name: str) -> None:
        ...
        if ".." in name or name.startswith("/") or name.startswith("\\"):
            raise ValueError(
                f"Security violation: Invalid template name '{name}' detected."
            )

        # Ensure name only contains alphanumerics, underscores, slashes, and periods
        if not re.match(r"^[a-zA-Z0-9_\-\.\/]+$", name):
            raise ValueError(
                f"Validation failure: Template name '{name}' contains illegal characters."
            )

    def render_html(
        self, template_name: str, context: Dict[str, Any], locale: str = "en"
    ) -> str:
