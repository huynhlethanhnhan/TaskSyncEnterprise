...
class Notification(Base):
    ...
    priority: Mapped[str] = mapped_column(
        Unicode(20), nullable=False, default="NORMAL", server_default=text("N'NORMAL'")
    )
    ...
    channel: Mapped[str] = mapped_column(
        Unicode(20), nullable=False, default="IN_APP", server_default=text("N'IN_APP'")
    )
    event_id: Mapped[Optional[str]] = mapped_column(
        Unicode(50), nullable=True, index=True
    )
    context_json: Mapped[Optional[str]] = mapped_column(UnicodeText, nullable=True)
    ...
    employee: Mapped["Employee"] = relationship(
        "Employee", back_populates="notifications", foreign_keys=[employee_id]
    )
