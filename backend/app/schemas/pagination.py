...
class BaseFilterParams(BaseModel):
    ...
    created_from: datetime | None = Field(default=None, description="Created from timestamp")
    created_to: datetime | None = Field(default=None, description="Created to timestamp")
    updated_from: datetime | None = Field(default=None, description="Updated from timestamp")
    updated_to: datetime | None = Field(default=None, description="Updated to timestamp")
...
class SortParams(BaseModel):
    ...
    def validate_sort_by(self, allowed_fields: List[str]) -> str | None:
        ...
        if self.sort_by and self.sort_by not in allowed_fields:
            raise ValueError(f"Sorting by '{self.sort_by}' is not allowed. Allowed fields: {allowed_fields}")
        return self.sort_by