...
class ResponseMeta(BaseModel):
    ...
    page: int | None = Field(default=None, description="Current page number (1-indexed)")
    ...
    total: int | None = Field(default=None, description="Total count of database records matching query")
    total_pages: int | None = Field(default=None, description="Total pages count (alias for compatibility)")
    has_next: bool | None = Field(default=None, description="Indicates if there is a next page")
    has_previous: bool | None = Field(default=None, description="Indicates if there is a previous page")
    ...
    def calculate_pagination(cls, data: Any) -> Any:
        if isinstance(data, dict):
            ...
            if page is not None and size is not None and total is not None:
                ...
                data["has_next"] = data.get("has_next") if data.get("has_next") is not None else (page < pages)
                data["has_previous"] = data.get("has_previous") if data.get("has_previous") is not None else (page > 1)
        return data
...
class SuccessResponse(BaseModel, Generic[T]):
    ...
    request_id: str | None = Field(default=None, description="Correlation identifier for tracing requests (Root-level for compatibility)")
    timestamp: str | None = Field(default=None, description="Response construction timestamp (Root-level for compatibility)")
    ...
    def sync_meta_and_root(cls, data: Any) -> Any:
        if isinstance(data, dict):
            ...
            if meta is None:
                ...
                meta["execution_time"] = meta.get("execution_time") or get_current_execution_time()
...
class CreatedResponse(SuccessResponse[T]):
    ...
    message: str = Field(default="Resource created successfully", description="User facing response message description")


class UpdatedResponse(SuccessResponse[T]):
    """Standard success API envelope for updated resources (200)."""
    message: str = Field(default="Resource updated successfully", description="User facing response message description")


class DeletedResponse(SuccessResponse[None]):
    """Standard success API envelope for deleted resources (200)."""
    message: str = Field(default="Resource deleted successfully", description="User facing response message description")
...
class PaginationModel(BaseModel, Generic[T]):
    ...
    items: list[T] = Field(default_factory=list, description="Array containing items payload collection")
...
class ErrorResponse(BaseModel):
    ...
    details: Any = Field(default=None, description="Detailed validation error payload blocks")
    trace_id: str | None = Field(default_factory=get_current_request_id, description="Correlation identifier for tracing requests")
    timestamp: str = Field(
        default_factory=lambda: datetime.now(timezone.utc).isoformat(),
        description="Response construction timestamp (UTC ISO format)"
    )
    # Backward compatibility fields
    request_id: str | None = Field(default=None, description="Correlation identifier for tracing requests (Compatibility)")
    data: Any = Field(default=None, description="Detailed validation error payload blocks (Compatibility)")
    ...
    def sync_error_fields(cls, data: Any) -> Any:
        if isinstance(data, dict):
            ...
            t_id = data.get("trace_id") or data.get("request_id") or get_current_request_id()
            data["trace_id"] = t_id
            data["request_id"] = t_id
            
            # Sync details and data
            dt = data.get("details") if data.get("details") is not None else data.get("data")
            data["details"] = dt
            data["data"] = dt
        return data
...
class ValidationErrorResponse(ErrorResponse):
    ...
    message: str = Field(default="Dữ liệu gửi lên không hợp lệ!", description="Detailed failure message details")
    error_code: str = Field(default="VALIDATION_REQUEST_FAILED", description="Module specific custom error identifier code")
