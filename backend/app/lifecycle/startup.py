...
def verify_redis_connection() -> None:
    ...
    if "pytest" in sys.modules or settings.ENVIRONMENT == "testing":
        app_logger.info(
            "Redis startup connectivity check bypassed in testing environment."
        )
        return
    ...
    try:
        ...
        if client.ping():
            ...
            app_logger.warning(
                "Redis connectivity verification: FAILED (ping returned False)"
            )
...
def run_startup() -> None:
    ...
    app_logger.info(
        "TaskSyncEnterprise startup validation sequence completed successfully."
    )
