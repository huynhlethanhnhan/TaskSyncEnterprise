...
class PrometheusMetrics:
    ...
    def __init__(self):
        ...
        self.app_uptime = Gauge(
            "app_uptime_seconds",
            "Application uptime duration in seconds"
        )
        ...
        self.db_requests_total = Counter(
            "db_requests_total",
            "Total database requests executed",
            ["statement_type"]
        )
        self.db_queries_successful = Counter(
            "db_queries_successful_total",
            "Total successful database queries executed"
        )
        ...
        self.tasks_created_total = Gauge(
            "tasks_created_total",
            "Total number of active tasks in the database"
        )
        self.projects_created_total = Gauge(
            "projects_created_total",
            "Total number of active projects in the database"
        )
        self.active_users_total = Gauge(
            "active_users_total",
            "Total number of active employees in the database"
        )
    ...
    def update_business_metrics(self, db: Session) -> None:
        ...
        with self._lock:
            try:
                ...
                tasks_count = db.execute(select(func.count(Task.id)).where(Task.is_deleted == False)).scalar() or 0
                projects_count = db.execute(select(func.count(Project.id)).where(Project.is_deleted == False)).scalar() or 0
                employees_count = db.execute(select(func.count(Employee.id)).where(Employee.is_deleted == False)).scalar() or 0
