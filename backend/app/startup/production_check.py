...
def run_production_readiness_check() -> dict:
    ...
    if settings.ENVIRONMENT == "production":
        if secret_key_val == "task_sync_enterprise_secret_key_chuandry_2026":
            app_logger.critical(
                "PRODUCTION AUDIT FAILURE: Weak/default SECRET_KEY is used in production!"
            )
            security_deductions += 50
        elif len(secret_key_val) < 32:
            app_logger.warning(
                "PRODUCTION AUDIT WARNING: SECRET_KEY is too short (<32 chars) for production."
            )
            security_deductions += 20

        if "*" in settings.ALLOWED_HOSTS:
            app_logger.warning(
                "PRODUCTION AUDIT WARNING: Wildcard '*' in ALLOWED_HOSTS is unsafe for production."
            )
