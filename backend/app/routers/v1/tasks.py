...
from app.core.deps import RequireManager, RequireEmployee, get_current_user # Thêm get_current_user để lấy vết audit
...
@router.put(
    "/{task_id:int}",
    response_model=TaskResponse,
    dependencies=[Depends(RequireManager)] # <-- Chỉ Admin và Manager được sửa toàn bộ Task
)
...
def get_my_tasks(
    current_user: Employee = Depends(RequireEmployee), # Cho cả 3 gọi, nhưng lôi ra User hiện tại để lọc DB
    db: Session = Depends(get_db)
):
...
@router.post("/{task_id}/attachments", summary="Đính kèm tài liệu, báo cáo vào Task công việc")
def upload_task_attachment(
    task_id: int,
    file: UploadFile = File(..., description="Chọn file đính kèm báo cáo, tài liệu dưới 20MB"),
    current_user: Employee = Depends(get_current_user), # Xác thực token để lấy thông tin người upload
    db: Session = Depends(get_db)
):
    # 1. Kiểm tra xem Task tồn tại vật lý trong hệ thống không
    task = crud_task.get_by_id(db, task_id)
    if task is None:
        raise HTTPException(status_code=404, detail="Không tìm thấy Task để đính kèm file!")
...
@router.delete("/{task_id}/attachments/{attachment_id}", summary="Xóa tài liệu đính kèm khỏi Task")
def delete_task_attachment(
    ...
    if attachment is None or attachment.task_id != task_id:
        raise HTTPException(status_code=404, detail="Không tìm thấy tài liệu đính kèm này cho Task!")
    ...
    if not (is_uploader or is_assigned or is_manager_or_admin):
        raise HTTPException(
            status_code=403,
            detail="Bạn không có quyền xóa tài liệu đính kèm này."
        )
