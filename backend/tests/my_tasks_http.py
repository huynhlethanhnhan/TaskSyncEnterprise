...
def test_user_my_tasks(email, password="123456"):
    ...
    user_info = body["user"]
    print(
        f"[PASS] Login successful! User ID: {user_info['id']} | Role ID: {user_info['role_id']} | Role Name: {user_info['role']}"
    )
    ...
    if res_tasks.status == 200:
        ...
        print(
            f"       Response Message: {body_tasks.get('message', 'N/A').encode('ascii', 'ignore').decode('ascii')}"
        )
