...
def test_email_sending_transient_retry_success():
    ...
    send_mock = MagicMock(side_effect=[smtplib.SMTPConnectError(500, "Transient error"), "Success"])
    ...
    with patch("app.services.email.smtp_client.SMTPClient.send", send_mock):
        ...
        with patch("time.sleep", return_value=None):
            result = service.send_email_with_retry(message, max_retries=2, backoff_base=0.1)
...
def test_email_sending_permanent_failure():
    ...
    send_mock = MagicMock(side_effect=smtplib.SMTPAuthenticationError(535, "Bad credentials"))
...
def test_email_sending_exhausted_retries():
    ...
    send_mock = MagicMock(side_effect=smtplib.SMTPConnectError(500, "Connection refused"))
