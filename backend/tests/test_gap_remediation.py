...
def test_sprint_lifecycle_and_backlog(client, db, setup_data):
    ...
    response = client.post(
        f"/api/v1/backlog/{backlog_id}/convert-to-task",
        headers=mgr_headers
    )
    assert response.status_code == 200
...
def test_topics_and_replies(client, db, setup_data):
    ...
    response = client.post(
        "/api/v1/topics",
        json={"project_id": proj_id, "title": "Design Discussion", "content": "How should we design this?"},
        headers=emp_headers
    )
    assert response.status_code == 201
    ...
    response_idor = client.post(
        "/api/v1/topics",
        json={"project_id": proj_id, "title": "Malicious Topic", "content": "No access"},
        headers=emp2_headers
    )
    assert response_idor.status_code == 403
