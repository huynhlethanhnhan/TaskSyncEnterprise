...
def test_unread_count_and_marking_read(db, repo_employee):
    ...
    n1 = notification_repo.create_notification(
        db=db, employee_id=repo_employee.id, type="SYSTEM", title="Alert 1", message="A", channel="IN_APP"
    )
    n2 = notification_repo.create_notification(
        db=db, employee_id=repo_employee.id, type="SYSTEM", title="Alert 2", message="B", channel="IN_APP"
    )
    ...
    n3 = notification_repo.create_notification(
        db=db, employee_id=repo_employee.id, type="SYSTEM", title="Alert 3", message="C", channel="EMAIL"
    )
...
def test_preferences_crud(db, repo_employee):
    ...
    pref_updated = notification_repo.get_user_preference(db, repo_employee.id, "TASKS", "EMAIL")
    assert pref_updated.enabled is False
    
    # 5. Delete preference
    deleted = notification_repo.delete_user_preference(db, repo_employee.id, "TASKS", "EMAIL")
    assert deleted is True
    assert notification_repo.get_user_preference(db, repo_employee.id, "TASKS", "EMAIL") is None


def test_notification_logs_crud(db, repo_employee):
    # Create parent notification
    notif = notification_repo.create_notification(
        db=db, employee_id=repo_employee.id, type="SYSTEM", title="Test Logs", message="X"
    )
...
def test_paginated_inbox_search(db, repo_employee):
    ...
    notification_repo.create_notification(
        db=db, employee_id=repo_employee.id, type="TASKS", title="Design Schema", message="Message 1"
    )
    notification_repo.create_notification(
        db=db, employee_id=repo_employee.id, type="TASKS", title="Review Code", message="Message 2"
    )
    notification_repo.create_notification(
        db=db, employee_id=repo_employee.id, type="VACATION", title="Submit Request", message="Message 3"
    )
