...
def test_logging_e2e_compliance(db, request):
    ...
    res_login = client.post(
        "/api/v1/auth/login",
        data={"username": "nonexistent_user@example.com", "password": "wrong_password"},
    )
    ...
    with patch("app.routers.v1.health.health_service.get_liveness") as mock_liveness:
        mock_liveness.side_effect = Exception(
            "Simulated unhandled 500 error for logging validation"
        )
    ...
    json_access_entries = [
        try_parse_json(line) for line in new_access_lines if line.strip()
    ]
    json_access_entries = [e for e in json_access_entries if e is not None]
    assert (
        len(json_access_entries) >= 1
    ), "access.log must contain at least one JSON entry"

    # B. Find "HTTP Request Completed" entries
    request_completed_entries = [
        e
        for e in json_access_entries
        if isinstance(e.get("message"), str)
        and "HTTP Request Completed:" in e["message"]
    ]
    assert (
        len(request_completed_entries) == 4
    ...
    assert (
        ...
        or any(
            "Simulated unhandled 500 error for logging validation"
            in (e.get("exception") or "")
            for e in json_error_entries
        )
        or any(
            e.get("level") in ("ERROR", "CRITICAL", "WARNING")
            and "500" in (e.get("message") or "")
            for e in json_error_entries
        )
    ...
    for line in new_app_lines:
        ...
        if parsed and parsed.get("logger") == "access":
            assert False, f"Access log entry must not appear in app.log: {line.strip()}"
    ...
    for entry in request_completed_entries:
        assert (
            entry.get("service_name") == settings.APP_NAME
