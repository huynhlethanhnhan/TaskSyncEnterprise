...
def run_test():
    ...
    try:
        ...
        new_noti = notification_crud.create(db, title=title, message=message, employee_id=user.id)
        print(f"[PASS] Created new notification. ID: {new_noti.id}, Title: '{new_noti.title}', is_read: {new_noti.is_read}")
        ...
        if found:
            print(f"[PASS] Successfully retrieved notification with ID {new_noti.id} from employee's notifications list.")
        else:
            print(f"[FAIL] Notification with ID {new_noti.id} not found in employee's notifications list.")

        # 4. Đánh dấu đã đọc
        updated_noti = notification_crud.mark_as_read(db, notification=new_noti)
        print(f"[PASS] Marked as read. ID: {updated_noti.id}, is_read: {updated_noti.is_read}")
