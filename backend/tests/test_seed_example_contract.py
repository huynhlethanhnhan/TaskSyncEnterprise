...
class SeedExampleContractTest(unittest.TestCase):
    def test_seed_example_has_enterprise_sized_dataset(self):
        ...
        self.assertGreaterEqual(EXPECTED_COUNTS["notifications"], EXPECTED_COUNTS["employees"] * 3)
    ...
    def test_seed_example_contains_unicode_and_management_assignments(self):
        ...
        manager_codes = {employee["employee_code"] for employee in plan["employees"] if employee["role"] == "manager"}

        self.assertIn("Huỳnh Lê Thành Nhân", names)
        self.assertEqual(len(manager_codes), EXPECTED_COUNTS["managers"])
        self.assertTrue(all(employee.get("manager_code") for employee in plan["employees"] if employee["role"] == "employee"))
        self.assertTrue(all(employee["email"].endswith("@tasksync.example.com") for employee in plan["employees"]))
        self.assertFalse(any(employee["email"].endswith(".local") for employee in plan["employees"]))
