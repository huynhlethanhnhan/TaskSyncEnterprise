...
def test_no_localhost_in_nginx_upstream():
    ...
    assert not localhost_proxy, "Found illegal 'proxy_pass http://localhost' in Nginx configuration! Must use Docker service DNS."
...
def test_required_forwarded_headers_exist():
    ...
    for header in required_headers:
        assert header in content, f"Missing required header setting in Nginx config: '{header}'"
...
def test_no_wildcard_forwarded_allow_ips():
    ...
    dockerfile_content = (REPO_ROOT / "backend" / "Dockerfile").read_text(encoding="utf-8")
    assert "--forwarded-allow-ips=*" not in dockerfile_content, "Backend Dockerfile must not hardcode '--forwarded-allow-ips=*'!"
    ...
    assert "FORWARDED_ALLOW_IPS=*" not in env_str, "docker-compose.production.yml must not use wildcard FORWARDED_ALLOW_IPS=*!"
...
def test_backend_network_has_fixed_subnet():
    ...
    assert len(ipam_config) > 0, "backend-network must define explicit IPAM config in docker-compose.production.yml!"
...
def test_env_production_example_safe_forwarded_allow_ips():
    ...
    assert "FORWARDED_ALLOW_IPS=172.30.0.0/24" in env_example, ".env.production.example must set FORWARDED_ALLOW_IPS=172.30.0.0/24"
    assert "FORWARDED_ALLOW_IPS=*" not in env_example, ".env.production.example must not suggest wildcard '*'"
...
def test_docker_compose_production_ports():
    ...
    assert len(nginx_ports) >= 2, "Nginx service must expose HTTP (80) and HTTPS (443) ports"

    backend_ports = services.get("backend", {}).get("ports")
    assert backend_ports is None, "Backend service must NOT publish ports directly to host in production!"

    frontend_ports = services.get("frontend", {}).get("ports")
    assert frontend_ports is None, "Frontend service must NOT publish ports directly to host in production!"
...
def test_gitignore_contains_tls_and_exceptions():
    ...
    required_exceptions = ["!nginx/ssl/.gitkeep", "!nginx/ssl/generate_self_signed_cert.ps1"]
...
def test_documentation_secret_safe_commands():
    ...
    guide_path = REPO_ROOT / "docs" / "learning" / "phase-3.8.6-nginx-reverse-proxy-https-guide-vi.md"
    if guide_path.exists():
        content = guide_path.read_text(encoding="utf-8")
        assert "config --quiet" in content or "config" not in content, "Documentation must use 'config --quiet' to prevent leaking secrets!"
        assert "curl.exe -I http://localhost/health\n" not in content, "Documentation must not use 'curl -I' for GET-only /health endpoint!"
        assert "fully non-root" not in content.lower(), "Documentation must not claim Nginx container runs fully non-root!"
        assert "master process" in content.lower(), "Documentation must accurately explain Nginx master (root) and worker (nginx) processes."
...
def test_no_tracked_private_keys():
    ...
    assert len(tracked_keys) == 0, f"Found private key files in repository: {tracked_keys}"
