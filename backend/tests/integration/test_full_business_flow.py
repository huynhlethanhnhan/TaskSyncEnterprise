...
def get_auth_header(email: str, user_id: int, role_id: int):
    token = create_access_token({"sub": str(user_id), "email": email, "role_id": role_id})
    return {"Authorization": f"Bearer {token}"}


def test_full_business_flow_sequence(client):
    db = SessionLocal()
    try:
        # Step 1: Login Admin
        admin = db.query(Employee).filter(Employee.email == "admin@tasksync.com").first()
        ...
        dept_resp = client.post(
            "/api/v1/departments",
            json={"department_code": "ENG", "name": "Engineering", "description": "Software Engineering"},
        ...
        # Create unassigned employee in different department
