...
*   **Password Policy:**
    *   The backend has no configurations or validation filters for password complexity, min/max length, or character requirements. In [auth.py](app/routers/v1/auth.py), user passwords are updated without security checks.
*   **Allowed Hosts:**
...
*   **SQL Connection URL Loopback:**
    *   In `app/core/settings.py`, the loopback database check and dynamic URI building have configurable host parameters: `if self.MSSQL_HOST in ("localhost", "127.0.0.1")`.
*   **Pool Settings:**
    *   In [database.py](app/database.py), pool recycling (`pool_recycle=1800`) and verification pre-pinging (`pool_pre_ping=True`) are hardcoded. Pool size limits and max overflow limits are not configured.
*   **Hardcoded Database Schema:**
    *   In [base.py](app/models/base.py), the schema name is hardcoded: `schema="dbo"`. This prevents changing the schema namespace dynamically between staging, testing, and production.
